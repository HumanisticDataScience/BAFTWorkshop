...
######################################
## EXAMINE TRAIN/TEST SETS VISUALLY
...
#Plot results
xyzresults%>%
  ggplot(aes(reorder(item_id, cv_value), cv_value, fill=forecastability))+
  geom_col()+
  scale_fill_brewer(type='qual', direction = 1)+
...
ets_forecast <- ets_models %>%
  mutate(fcast = map(ets_fit,
                     forecast,
                     h=365))%>% #add (365+2) more days to match test period exactly
  mutate(swp = map(fcast, sw_sweep, fitted=FALSE))%>%
  unnest(swp)%>%
  filter(key == 'forecast')%>%
  mutate(dt = seq(from = as.Date('2015-04-26'), by='day', length.out = 365))%>%
